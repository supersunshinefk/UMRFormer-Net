...
def infersTs_process_label(label):
    # rv = label == 1
    # myo = label == 2
    # lv = label == 3
    pancreas = label == 1
...
def test(fold):
    ...
    path='/disk7/fangkun/Synapse/UMRFormer_raw/UMRFormer_raw_data/UMRFormer_raw_data/Task004_pancreastumor/'
    label_list=sorted(glob.glob(os.path.join(path,'labelsTs/TCIA','*nii.gz')))
    infer_list=sorted(glob.glob(os.path.join(path,'infersTs/nnTrans_output/TCIA','*nii.gz')))
    ...
    if not os.path.exists(file):
        os.makedirs(file)
    fw = open(file+'/TCIA_pancreas_dice.txt', 'w')
    ...
    for label_path,infer_path in zip(label_list,infer_list):
        ...
        fw.write('hd95_pancreas: {:.4f}\n'.format(hd95_pancreas[-1]))
    ...
    fw.write('hd95_pancreas' + str(np.mean(hd95_pancreas)) + '\n')
    ...
    hd95.append(np.mean(hd95_pancreas))
