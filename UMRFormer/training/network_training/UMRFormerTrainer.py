...
from UMRFormer_Net.training.dataloading.dataset_loading import load_dataset, DataLoader3D, DataLoader2D, unpack_dataset
from UMRFormer_Net.training.loss_functions.dice_loss import DC_and_CE_loss
from UMRFormer_Net.training.network_training.network_trainer import NetworkTrainer
...
class UMRFormerTrainer(NetworkTrainer):
    def __init__(self, plans_file, fold, output_folder=None, dataset_directory=None, batch_dice=True, stage=None,
        ...
        IMPORTANT: If you inherit from UMRFormerTrainer and the init args change then you need to redefine self.init_args
        in your init accordingly. Otherwise checkpoints won't load properly!
        """
        super(UMRFormerTrainer, self).__init__(deterministic, fp16)
    ...
    def run_training(self):
        ...
        super(UMRFormerTrainer, self).run_training()
    ...
    def preprocess_patient(self, input_files):
        ...
        preprocessor_class = recursive_find_python_class([join(UMRFormer_Net.__path__[0], "preprocessing")],
                                                         preprocessor_name,
                                                         current_module="UMRFormer_Net.preprocessing")
        assert preprocessor_class is not None, "Could not find preprocessor %s in UMRFormer_Net.preprocessing" % \
                                               preprocessor_name
    ...
    def validate(self, do_mirroring: bool = True, use_sliding_window: bool = True, step_size: float = 0.5,
        ...
        if run_postprocessing_on_folds:
            # in the old UMRFormer_Net we would stop here. Now we add a postprocessing. This postprocessing can remove everything
    ...
    def run_online_evaluation(self, output, target):
        with torch.no_grad():
            ...
            output_seg = output_softmax.argmax(1)
            # print("!!!!!!!!!!!!!!!!!00000!!!!!!!!!!!!!!!!!!!!!", output_seg.shape,target.shape)
            target = target[:, 0]
            axes = tuple(range(1, len(target.shape)))
            i=0
            # ##UMRFormer source code ########
            #tp_hard = torch.zeros((target.shape[0], num_classes - 1)).to(output_seg.device.index)
            #fp_hard = torch.zeros((target.shape[0], num_classes - 1)).to(output_seg.device.index)
            #fn_hard = torch.zeros((target.shape[0], num_classes - 1)).to(output_seg.device.index)

            # Sypanse datasets ### 14 classes##### 8 -->>>Aotra Gallbladder Kidnery(L) Kidnery(R) Liver Pancreas Spleen Stomach
            ...
            # MSD datasets  ##### 3 classes ##no background## 2 ---->>>>>>pancreas, cancer
            tp_hard = torch.zeros((target.shape[0], 2)).to(output_seg.device.index)
            fp_hard = torch.zeros((target.shape[0], 2)).to(output_seg.device.index)
            fn_hard = torch.zeros((target.shape[0], 2)).to(output_seg.device.index)

            # ZJPancreas/ZJPancreasCancer datasets  ##### 1 classes ##no background## 1 ---->>>>>>pancreas or cancer
            # tp_hard = torch.zeros((target.shape[0], 1)).to(output_seg.device.index)
            # fp_hard = torch.zeros((target.shape[0], 1)).to(output_seg.device.index)
            # fn_hard = torch.zeros((target.shape[0], 1)).to(output_seg.device.index)
            ...
            # MSD and  ZJPancreas/ZJPancreasCancer datasets
            for c in range(1, num_classes):
                tp_hard[:, i] = sum_tensor((output_seg == c).float() * (target == c).float(), axes=axes)
                fp_hard[:, i] = sum_tensor((output_seg == c).float() * (target != c).float(), axes=axes)
                fn_hard[:, i] = sum_tensor((output_seg != c).float() * (target == c).float(), axes=axes)
    ...
    def finish_online_evaluation(self):
        ...
        # print("???????????????????", self.online_eval_tp, self.online_eval_fp, self.online_eval_fn)
        global_dc_per_class = [i for i in [2 * i / (2 * i + j + k) for i, j, k in
                                           zip(self.online_eval_tp, self.online_eval_fp, self.online_eval_fn)]
                               if not np.isnan(i)]
        self.all_val_eval_metrics.append(np.mean(global_dc_per_class))
        
        # writer.add_scalar('spleen_dice', global_dc_per_class[0])
        # writer.add_scalar('r_kidnery_dice', global_dc_per_class[1])
        # writer.add_scalar('l_kidnery_dice', global_dc_per_class[2])
        # writer.add_scalar('gallbladder_dice', global_dc_per_class[3])
        # writer.add_scalar('liver_dice', global_dc_per_class[4])
        # writer.add_scalar('stomach_dice', global_dc_per_class[5])
        # writer.add_scalar('aotra_dice', global_dc_per_class[6])
        # writer.add_scalar('pancreas_dice', global_dc_per_class[7])
    ...
    def save_checkpoint(self, fname, save_optimizer=True):
        super(UMRFormerTrainer, self).save_checkpoint(fname, save_optimizer)
        info = OrderedDict()
