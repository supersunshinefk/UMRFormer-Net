...
from UMRFormer_Net.experiment_planning.DatasetAnalyzer import DatasetAnalyzer
from UMRFormer_Net.experiment_planning.utils import crop
from UMRFormer_Net.paths import *
import shutil
from UMRFormer_Net.utilities.task_name_id_conversion import convert_id_to_task_name
from UMRFormer_Net.preprocessing.sanity_checks import verify_dataset_integrity
from UMRFormer_Net.training.model_restore import recursive_find_python_class
...
def main():
    ...
    parser.add_argument("-overwrite_plans_identifier", type=str, default=None, required=False,
                        help="If you set overwrite_plans you need to provide a unique identifier so that UMRFormer knows "
                             ...
                             "'UMRFormer_train CONFIG TRAINER TASKID FOLD -p UMRFormerPlans_pretrained_IDENTIFIER "
    ...
    for i in task_ids:
        ...
        if args.verify_dataset_integrity:
            verify_dataset_integrity(join(UMRFormer_raw_data, task_name))

        crop(task_name, False, tf)

        tasks.append(task_name)

    search_in = join(UMRFormer_Net.__path__[0], "experiment_planning")

    if planner_name3d is not None:
        planner_3d = recursive_find_python_class([search_in], planner_name3d, current_module="UMRFormer_Net.experiment_planning")
        if planner_3d is None:
            raise RuntimeError("Could not find the Planner class %s. Make sure it is located somewhere in "
                               "UMRFormer_Net.experiment_planning" % planner_name3d)
    else:
        planner_3d = None

    if planner_name2d is not None:
        planner_2d = recursive_find_python_class([search_in], planner_name2d, current_module="UMRFormer_Net.experiment_planning")
        if planner_2d is None:
            raise RuntimeError("Could not find the Planner class %s. Make sure it is located somewhere in "
                               "UMRFormer_Net.experiment_planning" % planner_name2d)
    else:
        planner_2d = None

    for t in tasks:
        print("\n\n\n", t)
        cropped_out_dir = os.path.join(UMRFormer_cropped_data, t)
        preprocessing_output_dir_this_task = os.path.join(preprocessing_output_dir, t)
        #splitted_4d_output_dir_task = os.path.join(UMRFormer_raw_data, t)
        ...
        shutil.copy(join(UMRFormer_raw_data, t, "dataset.json"), preprocessing_output_dir_this_task)
