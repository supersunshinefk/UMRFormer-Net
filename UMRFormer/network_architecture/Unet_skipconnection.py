...
def normalization(planes, norm='in'):
    if norm == 'bn':
        ...
        m = nn.InstanceNorm3d(planes,eps=0.00001,affine=True,momentum=0.1)
    else:
...
class InitConv(nn.Module):
    def __init__(self, in_channels=4, out_channels=16, dropout=0.):
...
class Unet(nn.Module):
    ...
    def forward(self, x):
        ...
        output = self.EnBlock5_4(x5_3)  # (1, 256, 8, 8, 8)
